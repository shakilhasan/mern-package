  ...
  const onSubmit = async () => {
    ...
    try {
      ...
      const finalProduct={
        ...
        isMostPopular:product.taxes,
        cityName:product.code,
        duration:product.sku,
        startingPrice:product.price,
        plans:[],
      };

      if(isEdit){
        await updatePackage(finalProduct);
      }else{
        delete finalProduct._id;
        await addPackage(finalProduct);
      }
      reset();
      ...
      <FormProvider methods={methods} onSubmit={handleSubmit(onSubmit)}>
        <Grid container spacing={3}>
          ...
          <Grid item xs={12} md={4}>
            <Stack spacing={3}>
              <Card sx={{ p: 3 }}>
                <RHFSwitch name="inStock" label="isActive" />
                <RHFSwitch name="taxes" label="isMostPopular" />

                <Stack spacing={3} mt={2}>
                  <RHFTextField name="code" label="cityName" />
                  <RHFTextField name="sku" label="duration" />
